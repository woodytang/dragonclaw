 ...
 */
export const PORTS = {
  /** DragonClaw GUI development server port */
  DragonClaw_DEV: 5173,

  /** DragonClaw GUI production port (for reference) */
  DragonClaw_GUI: 23333,

  /** Local host API server port */
  DragonClaw_HOST_API: 3210,
...
export function getPort(key: keyof typeof PORTS): number {
  const envKey = `DragonClaw_PORT_${key}`;
...
export const APP_PATHS = {
  ...
  /** DragonClaw configuration directory */
  DragonClaw_CONFIG: '~/.DragonClaw',

  /** Log files directory */
  LOGS: '~/.DragonClaw/logs',
} as const;
