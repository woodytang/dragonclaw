...
interface SyncProxyOptions {
   ...
   * currently disabled in DragonClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync DragonClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because DragonClaw proxy is disabled and preserve mode is enabled');
      return;
    }
