 ...
 * Used by DragonClaw to seed the provider store when it's empty but providers are
...
/**
 * Write the DragonClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged DragonClaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  DragonClaw sets
 ...
 * OpenClaw/DragonClaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When DragonClaw-managed moonshot provider exists, prefer centralized key
    ...
    // for DragonClaw to properly integrate with its updated tool system.
    ...
    // but DragonClaw historically stored them only under `channels.<type>.accounts.default`.
