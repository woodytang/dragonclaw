...
# Post-installation script for DragonClaw on Linux
...
# Create symbolic link for DragonClaw app binary
if [ -x /opt/DragonClaw/DragonClaw ]; then
    ln -sf /opt/DragonClaw/DragonClaw /usr/local/bin/DragonClaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/DragonClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/DragonClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/DragonClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/DragonClaw/resources/apparmor-profile'
...
echo "DragonClaw has been installed successfully."
